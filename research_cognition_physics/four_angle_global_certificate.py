"""Round 166: certify all independent inputs for the four-angle recovery.
...
from four_angle_history_decoder import PARAMETERS, branch_errors, error_jet, gram_from_sigma
...
from compiled_soft_history_recovery import report_probabilities, MODIFIED
from soft_decoder_certificate import (coefficients, DENOMINATOR, partial_reference,
    block_expectation, LOWER as PREVIOUS_LOWER)
from soft_history_decoder import branch_weights, schur_root


UPPER = F(44939475, 10**8)
LOWER = F(4493947394, 10**10)
COVER_PATH = Path(__file__).with_name('four_angle_global_cover.json')
WITNESS_PATH = Path(__file__).with_name('four_angle_witness.json')
...
def witness_data():
    ...
    if data['parameters_exact'] != [str(t) for t in PARAMETERS]:
...
def verify_cover(data):
    ...
    if data.get('parameters_exact') != [str(t) for t in PARAMETERS]:
    ...
    return {'round':166, 'parameters_exact':[str(t) for t in PARAMETERS],
            ...
            'previous_common_angle_legal_lower_exact':str(PREVIOUS_LOWER),
            'strict_improvement_over_common_angle_at_least_exact':str(PREVIOUS_LOWER-UPPER),
            ...
            'new_native_gates_worst':413, 'coherent_history_rebits':1,
            'expected_gates_same_as_round_164':True, 'certificate_requires_scipy':False,
            'limited_angle_tolerance':angle_certificate(),
            'optimization_over_four_angles_proved_here':False}


def angle_certificate(epsilon=F(1, 2000)):
    if type(epsilon) not in (int, F) or epsilon < 0:
        raise ValueError('Use a nonnegative rational rotation tolerance.')
    p = report_probabilities(majority_error(1))
    bound = I.exact(UPPER)+epsilon*sum(p[g] for g in MODIFIED)
    return {'two_sandwich_rotations_each_tolerance_radians_exact':str(F(epsilon)),
            'external_error_upper':interval_fields(bound),
            'beats_previous_ideal_common_angle_scheme':F(bound.hi,SCALE) < PREVIOUS_LOWER,
            'remaining_improvement':interval_fields(PREVIOUS_LOWER-bound),
            'all_other_gates_readout_and_coherent_storage_ideal':True,
...
def generate_cover():
    ...
    report = {'format':1,'root':'[-1,1] x [0,1] x [-1,1]',
              'parameters_exact':[str(t) for t in PARAMETERS], 'upper_exact':str(UPPER),
...
class FourAngleGlobalCertificateTests(unittest.TestCase):
    def test_saved_cover_proves_strict_improvement(self):
        result = verify_cover(json.loads(COVER_PATH.read_text(encoding='utf-8')))
        self.assertGreater(F(result['strict_improvement_over_common_angle_at_least_exact']), F(468,10**6))
        self.assertTrue(result['all_mixed_independent_sources_covered'])
        self.assertTrue(result['limited_angle_tolerance']['beats_previous_ideal_common_angle_scheme'])
    ...
    def test_wrong_parameters_and_coefficients_are_rejected(self):
        data = json.loads(COVER_PATH.read_text(encoding='utf-8')); data['parameters_exact'][0]='0'
...
def main():
    ...
    checks=unittest.TextTestRunner(verbosity=2).run(unittest.defaultTestLoader.loadTestsFromTestCase(FourAngleGlobalCertificateTests))
    ...
    if args.write_results:
        Path(__file__).with_name('four_angle_global_certificate_results.json').write_text(json.dumps(report,indent=2)+'\n',encoding='utf-8')
